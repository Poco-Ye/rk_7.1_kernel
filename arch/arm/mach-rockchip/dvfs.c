...
static int rockchip_get_irdrop(struct scaling_sel_table *board_irdrop_tbl,
			       unsigned long rate)
{
	int i;
	int board_irdrop = -EINVAL;

	if (!board_irdrop_tbl)
		return -EINVAL;

	rate /= 1000;
	for (i = 0; board_irdrop_tbl[i].sel != CPUFREQ_TABLE_END; i++) {
		if (rate >= board_irdrop_tbl[i].min)
			board_irdrop = board_irdrop_tbl[i].sel;
		else
			break;
	}
...
static void rockchip_adjust_opp_by_irdrop(struct dvfs_node *clk_dvfs_node)
{
	struct cpufreq_frequency_table *dvfs_table;
	struct scaling_sel_table *board_irdrop_tbl;
	int evb_irdrop = 0, board_irdrop = 0, delta_irdrop;
	unsigned int safe_frequency = 0;
	bool reach_max_volt = false;
	int max_volt, i;

	if (!clk_dvfs_node->dvfs_table || !clk_dvfs_node->max_volt)
		return;

	max_volt = clk_dvfs_node->max_volt;
	evb_irdrop = clk_dvfs_node->evb_irdrop;
	dvfs_table = clk_dvfs_node->dvfs_table;
	board_irdrop_tbl = clk_dvfs_node->board_irdrop_tbl;
	for (i = 0; (dvfs_table[i].frequency != CPUFREQ_TABLE_END); i++) {
		board_irdrop = rockchip_get_irdrop(board_irdrop_tbl,
						   dvfs_table[i].frequency);
		...
		if ((dvfs_table[i].index + delta_irdrop) <= max_volt) {
			dvfs_table[i].index += delta_irdrop;
			if (!reach_max_volt)
				safe_frequency = dvfs_table[i].frequency;
			if (dvfs_table[i].index == max_volt)
				reach_max_volt = true;
		} else {
			dvfs_table[i].index = max_volt;
		}
	}
	if (strcmp(clk_dvfs_node->name, "clk_core"))
		return;
	if (safe_frequency != dvfs_table[i - 1].frequency) {
		rockchip_pll_clk_adaptive_rate(safe_frequency * 1000);
	}
}
...
static int initialize_dvfs_node(struct dvfs_node *clk_dvfs_node)
{
	...
	rockchip_adjust_opp_by_irdrop(clk_dvfs_node);
	dvfs_table_round_volt(clk_dvfs_node);
			      ...
			      struct dvfs_node *dvfs_node)
{
	...
	dvfs_node->scaling_sel_tbl = of_get_sel_table(np,
						      "leakage-scaling-sel");
	dvfs_node->board_irdrop_tbl = of_get_sel_table(np,
						       "board-irdrop");
	of_property_read_u32_index(np, "max-volt", 0, &dvfs_node->max_volt);
	of_property_read_u32_index(np, "evb-irdrop", 0, &dvfs_node->evb_irdrop);
